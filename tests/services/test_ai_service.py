...
def ai_service(mock_env_vars, mock_mistral_client):
    ...
    with patch('src.services.ai_service.MistralClient', return_value=mock_mistral_client):
...
def test_generate_response_with_custom_model(ai_service):
    ...
    messages = [
        {"role": "user", "content": "Bonjour!"},
        {"role": "assistant", "content": "Bonjour! Comment puis-je vous aider?"},
        {"role": "user", "content": "Explique-moi la physique quantique en termes simples"}
    ]
