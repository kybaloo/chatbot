...
def test_save_conversation(mock_conversation_repository, sample_conversation):
    ...
    assert call_args["Item"]["SK"] == f"CONVERSATION#{sample_conversation.conversation_id}"
...
def test_list_conversations_for_user(mock_conversation_repository, sample_conversation):
    ...
    assert call_args["KeyConditionExpression"].expression == "PK = :pk AND begins_with(SK, :sk_prefix)"
    assert call_args["ExpressionAttributeValues"][":pk"] == f"USER#{sample_conversation.user_id}"
