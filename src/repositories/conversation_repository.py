...
class ConversationRepository(BaseRepository[Conversation]):
    ...
    def __init__(self):
        ...
        if env_vars.AWS_PROFILE:
            ...
            self.dynamodb = session.resource('dynamodb', region_name=env_vars.AWS_REGION_NAME)
        else:
            self.dynamodb = boto3.resource('dynamodb', region_name=env_vars.AWS_REGION_NAME)
        
        self.table = self.dynamodb.Table(env_vars.DYNAMO_TABLE)
        self.logger.info(f"ConversationRepository initialized with table {env_vars.DYNAMO_TABLE}")
    ...
    def _convert_to_item(self, entity: Conversation) -> Dict[str, Any]:
        ...
        item = {
            ...
            'ttl': int((datetime.now() + timedelta(days=env_vars.CONVERSATION_TTL_DAYS)).timestamp())
        }
        return item
    ...
    async def get_by_user_and_conversation(self, user_id: str, conversation_id: str) -> Optional[Conversation]:
        ...
        try:
            ...
            self.logger.error(f"Error getting conversation {conversation_id} for user {user_id}: {str(e)}")
            return None
    ...
    async def delete_by_user_and_conversation(self, user_id: str, conversation_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error deleting conversation {conversation_id} for user {user_id}: {str(e)}")
            return False
