...
class TelegramBot:
    ...
    def _initialize_application(self):
        ...
        self.application.add_handler(CallbackQueryHandler(self.handle_model_selection, pattern=r"^model_"))
        self.application.add_handler(CallbackQueryHandler(self.handle_settings_selection, pattern=r"^settings_"))
    ...
    async def history_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = []
        for conv in conversations[:10]:  # Limiter à 10 conversations pour éviter un clavier trop grand
            # Créer un résumé de la conversation
            summary = create_conversation_summary(conv)
            keyboard.append([InlineKeyboardButton(summary, callback_data=f"conv_{conv.conversation_id}")])
        
        # Ajouter un bouton pour créer une nouvelle conversation
        keyboard.append([InlineKeyboardButton("➕ Nouvelle conversation", callback_data="conv_new")])
    ...
    async def settings_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = [
            [InlineKeyboardButton("🤖 Changer de modèle d'IA", callback_data="settings_model")],
            [InlineKeyboardButton("🌍 Changer de langue", callback_data="settings_language")],
            [InlineKeyboardButton("📝 Style de réponse", callback_data="settings_style")],
    ...
    async def handle_conversation_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        ...
        conversation = await self.conversation_service.get_by_user_and_conversation(user_id, conversation_id)
        ...
        preview_messages = conversation.messages[-max_preview_messages:] if len(conversation.messages) > max_preview_messages else conversation.messages
        ...
        keyboard = [
            [
                InlineKeyboardButton("📝 Renommer", callback_data=f"settings_rename_{conversation_id}"),
                InlineKeyboardButton("🗑️ Supprimer", callback_data=f"settings_delete_{conversation_id}")
            ],
            [InlineKeyboardButton("📚 Historique", callback_data="conv_history")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def handle_model_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "model_list":
            ...
            keyboard.append([InlineKeyboardButton("⬅️ Retour", callback_data="settings_main")])
            reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def handle_settings_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        ...
        if data == "settings_model":
            ...
            success = await self.conversation_service.delete_by_user_and_conversation(user_id, conversation_id)
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not conversation:
            ...
            conversation = await self.conversation_service.create_new_conversation(user_id, user.username)
            self.active_conversations[user.id] = conversation.conversation_id
        ...
        try:
            ...
            assistant_response = await self.ai_service.process_conversation(conversation)
            ...
            await self.conversation_service.update(conversation.conversation_id, conversation)
