...
class ConversationService(BaseService[Conversation]):
    ...
    async def get_by_user_and_conversation(self, user_id: str, conversation_id: str) -> Optional[Conversation]:
        """Récupère une conversation spécifique d'un utilisateur"""
        return await self.repository.get_by_user_and_conversation(user_id, conversation_id)
    ...
    async def create_new_conversation(self, user_id: str, username: Optional[str] = None) -> Conversation:
    ...
    async def delete_by_user_and_conversation(self, user_id: str, conversation_id: str) -> bool:
        """Supprime une conversation spécifique d'un utilisateur"""
        return await self.repository.delete_by_user_and_conversation(user_id, conversation_id)
    
    async def add_message(self, conversation: Conversation, role: str, content: str) -> Conversation:
    ...
    async def clear_conversation_history(self, user_id: str, conversation_id: str) -> Optional[Conversation]:
    ...
    async def rename_conversation(self, user_id: str, conversation_id: str, new_title: str) -> Optional[Conversation]:
    ...
    def get_formatted_history(self, conversation: Conversation, max_messages: int = 5) -> str:
        ...
        messages = conversation.messages[-max_messages:] if len(conversation.messages) > max_messages else conversation.messages
